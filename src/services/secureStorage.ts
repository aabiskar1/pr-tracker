...
const getEncryptionKey = async (userPassword: string): Promise<CryptoKey> => {
  ...
  const { [SALT_KEY]: existingSalt } = await browser.storage.local.get(SALT_KEY);
...
export const validatePassword = async (password: string): Promise<boolean> => {
  try {
    const { [TEST_KEY]: testVector } = await browser.storage.local.get(TEST_KEY);
...
export const encryptToken = async (token: string, password: string): Promise<void> => {
  try {
    const key = await getEncryptionKey(password);
    const iv = generateIV();
...
export const decryptToken = async (password: string): Promise<string | null> => {
  try {
    const { [TOKEN_KEY]: encryptedArray, [IV_KEY]: ivArray } = await browser.storage.local.get([TOKEN_KEY, IV_KEY]);
...
export const hasStoredToken = async (): Promise<boolean> => {
  const { [TOKEN_KEY]: encryptedToken } = await browser.storage.local.get(TOKEN_KEY);
