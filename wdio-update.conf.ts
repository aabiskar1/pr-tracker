...
export const config: WebdriverIO.Config = {
    ...
    runner:
        process.env.TEST_TYPE === 'extension'
            ? 'local' // Use local runner for extension tests
            : [
                  'browser',
                  {
                      preset: 'react',
    ...
    specs:
        process.env.TEST_TYPE === 'extension'
            ? ['./e2e/Extension*.test.ts', './e2e/ExtensionBrowserTest.ts'] // Extension test specs
            : ['./e2e/Component.test.tsx'], // Component test specs
    // Patterns to exclude.
    exclude: [
    ...
    before: async function (capabilities, specs) {
        // Set up extension ID detection for extension tests
        if (process.env.TEST_TYPE === 'extension') {
            ...
            await browser.addCommand('getExtensionId', async function () {
                ...
                throw new Error(
                    `Could not determine extension ID for ${process.env.BROWSER}`
                );
            });
        }
    },
